...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fasta_file",
        type=str,
        default="../Predict/3gdt-a.fasta",
        help="Fasta file with protein sequences.",
    )
    parser.add_argument(
        "--task", type=str, default="mf", choices=["bp", "mf", "cc"], help=""
    )
    parser.add_argument(
        "--model_path", type=str, default="../Model/model_mf.pt", help=""
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="../Predict/",
        help="Save predictions/saliency in file.",
    )
    parser.add_argument(
        "-v", "--verbose", help="Prints predictions.", action="store_true"
    )
    parser.add_argument(
        "--use_guided_grads",
        default=True,
        help="Use guided grads to compute gradCAM.",
        action="store_true",
    )
    parser.add_argument(
        "--saliency",
        default="mf",
        help="Compute saliency maps for every protein and every MF-GO term/EC number.",
        action="store_true",
    )
    ...
    predictor.predict(
        args.fasta_file, args.task, args.pooling, args.contrast, args.device, args
    )

    # save predictions
    predictor.export_csv(
        args.output_fn_prefix + args.task.upper() + "_predictions.csv", args.verbose
    )
    predictor.save_predictions(
        args.output_fn_prefix + args.task.upper() + "_pred_scores.json"
    )

    # save saliency maps
    if args.saliency and args.task in ["mf", "ec"]:
        predictor.compute_GradCAM(use_guided_grads=args.use_guided_grads)
        predictor.save_GradCAM(
            args.output_fn_prefix + "_" + args.task.upper() + "_saliency_maps.json"
        )

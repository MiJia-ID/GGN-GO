...
class ProteinGraphDataset(data.Dataset):
    ...
    def __init__(self, dataset, index, args, task_list,
                 ...
                 top_k=50, num_rbf=16, device="cpu"):
        ...
        self.output_prottrans = args.prottrans_output_path 
        self.output_esm2 = args.esm2_output_path            
        self.proc_pdb_output_path = args.proc_pdb_output_path  
        self.pdb_dir = args.pdb_dir                         
        self.output_dssp = args.dssp_output_path             
    ...
    def _featurize_as_graph(self, idx):
        name = self.IDs[idx]
        with torch.no_grad():
            coords = np.load(self.proc_pdb_output_path + name + ".npy",allow_pickle=True)

            coords = torch.as_tensor(coords, device=self.device, dtype=torch.float32)

            seq = torch.as_tensor([self.letter_to_num[aa] for aa in self.dataset[name]],
                                  device=self.device, dtype=torch.long)  # self.dataset[name][0]
            ...
            prottrans_feat = torch.tensor(np.load(self.output_prottrans + name + ".npy",allow_pickle=True))
            esm_feat = torch.tensor(np.load(self.output_esm2 + name + ".npy",allow_pickle=True))
            dssp = torch.tensor(np.load(self.output_dssp + name + "_dssp.npy",allow_pickle=True))


            node_s = torch.cat([dihedrals, prottrans_feat, esm_feat,dssp], dim=-1).to(torch.float32)  # [**,2324]
            ...
            y_true = np.stack(prot2annot[name][self.task_list])
            y = torch.as_tensor(y_true, device=self.device, dtype=torch.float32)

...
parser.add_argument("--fasta_file", type=str, default='../data/nrPDB-GO_test_sequences.fasta')
...
def test(config, task, suffix):
    ...
    test_data = ProteinGraphDataset(test_fasta_data, range(len(test_fasta_data)), args_data, task_list=task)
    val_loader = DataLoader(test_data, batch_size=1, shuffle=False, drop_last=False, num_workers=args_data.num_workers,
                            prefetch_factor=2)

    output_dim = output_dim_dict[task]

    model = GGN(2324, 512, 3, 0.95, 0.2, output_dim, config.pooling, pertub=config.contrast).to(config.device)
